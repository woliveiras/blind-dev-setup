...
func TestMiseToolchainInstallsAndProbesPinnedTools(t *testing.T) {
	...
	wantInstall := []string{
		"install", "--yes", "python@3.14.7", "node@24.19.0", "uv@0.12.5", "pnpm@11.22.0",
	}
	if !slices.Equal(runner.commands[0].arguments, wantInstall) {
		t.Fatalf("install arguments = %#v", runner.commands[0].arguments)
	}
	wantProbes := [][]string{
	...
	for index, want := range wantProbes {
		if !slices.Equal(runner.commands[index+2].arguments, want) {
			t.Errorf("probe %d = %#v, want %#v", index, runner.commands[index+2].arguments, want)
		}
	}
